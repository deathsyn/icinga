...
when :apache
  ...
  Chef::Log.fatal("Unknown web server option provided for Nagios server: " <<
    "#{node['icinga']['server']['web_server']} provided. Allowed: :nginx or :apache"
  )
  raise 'Unknown web server option provided for Nagios server'
end

# Install icinga either from source of package
include_recipe "icinga::server_#{node['icinga']['server']['install_method']}"

sysadmins = search(:users, 'groups:sysadmin')

case node['icinga']['server_auth_method']
when "openid"
  if(web_srv == :apache)
    include_recipe "apache2::mod_auth_openid"
  else
    Chef::Log.fatal("OpenID authentication for Nagios is not supported on NGINX")
    Chef::Log.fatal("Set node['icinga']['server_auth_method'] attribute in your role: #{node['icinga']['server_role']}")
    raise
  end
else
...
directory "#{node['icinga']['conf_dir']}/dist" do
  owner node['icinga']['user']
  group node['icinga']['group']
  mode 00755
end

directory node['icinga']['state_dir'] do
  owner node['icinga']['user']
  group node['icinga']['group']
  mode 00751
end

directory "#{node['icinga']['state_dir']}/rw" do
  owner node['icinga']['user']
  group web_group
  mode 02710
end
...
execute "archive-default-nagios-object-definitions" do
  command "mv #{node['icinga']['config_dir']}/*_nagios*.cfg #{node['icinga']['conf_dir']}/dist"
  not_if { Dir.glob("#{node['icinga']['config_dir']}/*_nagios*.cfg").empty? }
end

directory "#{node['icinga']['conf_dir']}/certificates" do
  owner web_user
  group web_group
  mode 00700
end
...
bash "Create SSL Certificates" do
  cwd "#{node['icinga']['conf_dir']}/certificates"
  code <<-EOH
  umask 077
  openssl genrsa 2048 > nagios-server.key
  openssl req -subj "#{node['icinga']['ssl_req']}" -new -x509 -nodes -sha1 -days 3650 -key nagios-server.key > nagios-server.crt
  cat nagios-server.key nagios-server.crt > nagios-server.pem
  EOH
  not_if { ::File.exists?("#{node['icinga']['conf_dir']}/certificates/nagios-server.pem") }
end
...
service "icinga" do
  service_name node['icinga']['server']['service_name']
...
nagios_nrpecheck "check_nagios" do
  command "#{node['icinga']['plugin_dir']}/check_nagios"
  parameters "-F #{node["icinga"]["cache_dir"]}/status.dat -e 4 -C /usr/sbin/#{node['icinga']['server']['service_name']}"
  action :add
end
